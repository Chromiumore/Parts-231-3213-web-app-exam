    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if current_user.is_authenticated:
                ...
                if role_name == 'admin' or ((recipe is None or recipe.user_id == user_id) and (feedback is None or feedback.user_id == user_id) and not only_admin):
...
def view(recipe_id):
    ...
    return render_template('view-recipe.html', recipe=recipe, author=author, feedbacks_info=feedbacks_info, files=files)
...
def delete(recipe_id):
    ...
    is_successful = recipe_repository.delete(recipe_id)
    ...
    if is_successful:
        ...
        flash('Не удалось удалить рецепт', 'danger')
...
def delete_feedback(recipe_id, feedback_id):
    ...
    is_successful = feedback_repository.delete(feedback_id)
